...
for f in "$GENE_DIR"/*.faa; do [[ -e "$f" ]] || continue; b="$(basename "$f" .faa)"; exec_annotation --profile="$KOFAM_PROFILES" --ko-list="$KOFAM_KO_LIST" --cpu="$THREADS" --format=detail-tsv --output="$KOFAM_DIR/$b.kofam.tsv" "$f"; done
